...
research outlook on transcendentals $\pi$ and $e$ in HoTT via Cauchy reals
as a higher inductive--inductive type, directed HoTT and synthetic
...
correspondence.'' \Cref{sec:applications} sketches two downstream
applications: transcendentals $\pi$ and $e$ in HoTT and directed HoTT for
representation theory. \Cref{sec:open} collects open problems. \Cref{sec:conclusion}
concludes.
...
\node (n1) {(I) Naive};
\node (n2) [right=of n1] {(II) Set-th.};
\node (n3) [right=of n2] {(III) U.P.};
\node (n4) [right=of n3] {(IV) Yoneda};
\node (n5) [right=of n4] {(V) HoTT};
\node (n6) [right=of n5] {(VI) Struct.};
\draw[->] (n1) -- node[label] {T1} (n2);
\draw[->] (n2) -- node[label] {T2} (n3);
\draw[->] (n3) -- node[label] {T3} (n4);
\draw[->] (n4) -- node[label] {T4} (n5);
\draw[->] (n5) -- node[label] {T5} (n6);
\end{tikzpicture}%
}
